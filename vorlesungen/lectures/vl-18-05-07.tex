...
\]
). Gemacht: Schritt 1 und 2 (das waren die beiden insbesondere-Teile i) und ii)).

3. Schritt: zeige durch Induktion (Übungsaufgabe), dass 
\[
...
\[
	= \biggl(\sum_{d \vert (m',n')} d^{k-1} T\Bigl(\frac {m'n'}{d^2}\Bigr)\biggr) \biggl(\sum_{t \vert (p^r, p^s)} t^{k-1} T(\frac{p^rp^s}{t^2}) \biggr)
\]
und nach Schritt 1
\[
	= \sum_{\substack{d \vert (m',n')\\t \vert (p^r, p^s)}} (dt)^{k-1} T\Bigl(\frac {p^rm'p^sn'}{(dt)^2}\Bigr)
\]
und mit $D = dt$
\[
	= \sum_{D \vert (m,n)} D^{k-1} T\Bigl(\frac {mn}{D^2}\Bigr)
\]
