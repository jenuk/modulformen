...
und daher 
\[
	\smashoperator{\sum_{\substack{(c,d)\in\ZZ^2 \\ \ggt(c,d) = 1 \\ ad-bc = 1}}} \, \abs{cz+d}^{-k} \cdot \abs{e^{2\pi i n \frac{az+b}{cz+d}}} \leq \smashoperator{\sum_{\substack{(c,d)\in\ZZ^2 \\ \ggt(c,d) = 1 \\ ad-bc = 1}}} \, \abs{cz+d}^{-k}
	\,,
\]
...
\[
	P_n(z) = \frac 12 \smashoperator[r]{\sum_{M \in \linksmodulo{\Gamma(1)_\infty}{\Gamma(1)}}} \, (e^n |_k M)(z)
\]
...
\[
	K(m,n,c) := \smashoperator{\sum_{\substack{d (\operatorname{mod} c) \\ (c,d)=1}}} e^{2\pi i \frac{md + n\bar{d}}{c}}
	\,,
\]
...
Nach Definition ist 
\[
	P_n(z) = \frac 12 \smashoperator[r]{\sum_{\substack{(c,d)\in\ZZ^2 \\ \ggt(c,d) = 1 \\ ad-bc = 1}}} \, (cz+d)^{-k} e^{2\pi in \frac{az+b}{cz+d}}
	\,.
\]
...
\[
	\smashoperator{\sum_{\substack{c \geq 1, d \in \ZZ \\ \ggt(c,d) = 1 \\ ad-bc = 1}}} \, (cz+d)^{-k} e^{2 \pi i n \frac{az+b}{cz+d}} = \sum_{c \geq 1} \sum_{\substack{d' (\operatorname{mod} c) \\ \ggt(c,d') = 1 \\ ad'-b'c = 1}} \sum_{\nu \in \ZZ} \, (c(z + \nu) +d')^{-k} e^{2\pi in \frac{a(z+\nu)+b'}{c(z+\nu)+d'}}
	\,.
\]
...
\[
c(m) = \int_{ic}^{ic+1} \left( \sum_{\nu \in \ZZ} (z+\nu)^{-k} e^{-2\pi i \gamma \frac{1}{z+\nu}} \right) e^{-2\pi imz} \opd z
...
\[
P_n(z) = e^{2\pi nz} + \sum_{c\geq1} \smashoperator[r]{\sum_{\substack{d \bmod c\\ \ggt(d,c) = 1}}} \frac{2\pi (-1)^{\frac{k}{2}}}{c} \sum_{m\geq 1} \Bigl(\frac{m}{n}\Bigr)^{\frac{k-1}{2}} J_{k-1} \Bigl(\frac{4\pi \sqrt{mn}}{c}\Bigr) \cdot e^{\frac{2\pi i}{c}(na+md)} e^{2\pi imz}
\,.
\]
...
Nach Definition ist
\[
P_m = \frac{1}{2} \smashoperator[r]{\sum_{M\in \linksmodulo{\Gamma(1)_\infty}{\Gamma(1)}}} e^m |_k M
\]
unabhängig vom Vertretersystem von $\linksmodulo{\Gamma(1)_\infty}{\Gamma(1)}$.
Es gilt
\[
2 P_m | T(n) = n^{\frac{k}{2}-1} \smashoperator{\sum_{\substack{M \in \linksmodulo{\Gamma(1)_\infty}{\Gamma(1)}\\ N \in \linksmodulo{\Gamma(1)}{\mathcal{M}(n)}}}} e^m|_k MN
= n^{\frac{k}{2}-1} \smashoperator{\sum_{R \in \linksmodulo{\Gamma(1)_\infty}{\mathcal{M}(n)}}} e^m|R
\]
...
Es gilt nun
\[
	2 P_m | T(n) = n^{\frac k2 - 1} \smashoperator[l]{\sum_{M \in \linksmodulo {\Gamma(1)_\infty}{\Gamma(1)}}} \left( \smashoperator[r]{\sum_{\substack{ad = n, d > 0 \\ b (\operatorname{mod} d)}}} e^m |_k \mymat* ab0d \right) |_k M
	\,.
\]
Die innere Summe ist gleich 
\[
	\smashoperator{\sum_{\substack{d|n \\ b (\operatorname{mod} d)}}} n^{\frac k2} d^{-k} e^{2\pi im \left( \frac {n}{d^2} z + \frac bd \right)} = n^{\frac k2} \sum_{d | (m,n)} d^{1-k} e^{2\pi i \frac {mn}{d^2} z}
\]
wegen 
\[
	\smashoperator{\sum_{b (\operatorname{mod} d)}} e^{2\pi i \frac bd m} = \begin{cases} d, &d|m \\ 0, &\text{sonst}\end{cases}
	\,.
\]
Hieraus folgt die Behauptung.
\end{bewe}
