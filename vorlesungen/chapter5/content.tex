...
= m^{- \frac k2} \sum_{M \in \mathcal{M}(m)} (z'+z)^{-k}|_{k,z} M
\,.
\]
...
\begin{bewe}
	...
	&= m^{\frac k2 - 1} \sum_{\substack{M \in \linksmodulo{\Gamma(1)}{\M(m)}\\ \gamma \in \Gamma(1)}} (z'+z)^{-k} |_{k,z} \gamma M \\
	&= m^{\frac k2 - 1} \sum_{R \in \M(m)} (z'+z)^{-k}|_{k,z} R \\
	&= m^{k-1} h_m(z,z')
	\,.
	\end{split}
	\]
